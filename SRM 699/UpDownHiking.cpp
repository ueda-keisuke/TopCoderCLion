...
class UpDownHiking {
    ...
    int maxHeight(int N, int A, int B) {
        ...
        for( int ascend = 1; ascend < N; ascend++ )
        {
            int descend = N - ascend;
            int altitudes = min( A * ascend, B * descend );
            maxAltitudes = max( maxAltitudes, altitudes );
        }
